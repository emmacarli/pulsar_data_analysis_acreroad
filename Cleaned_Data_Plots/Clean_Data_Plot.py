...
raw_files = glob.glob(path_to_raw_files+'*-PSRB0329-2ms-sampling-dd.dat') #list of raw files on my computer

bar = Bar('Processing...', max=len(raw_files), fill='\U0001F4E1', suffix = '%(percent).1f%% - %(eta)ds') #create a progress bar
bar.check_tty = False

for raw_file in raw_files: #go through each raw file
    ...
    try:
        file_cleaned = path_to_cleaned_files +str(start_time_GPS)+'_cleaned.dat'
    ...
    start_time_GPS_astropy = Time(start_time_GPS, format='gps', precision=6, scale='utc') 
