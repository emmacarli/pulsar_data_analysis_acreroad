...
PRESTO_TOA_SNR_handle = open('PRESTO_TOAs_SNRs.txt' , 'w')
...
#ADD A COLUMN WITH GPS DATE or MJD
TOA_list = np.genfromtxt('TOAs_TEMPO_format.txt')[:,2]
FFTFIT_results = np.genfromtxt('FFTFIT_results.txt')
FFTFIT_results_with_MJDs = np.zeros((len(TOA_list),np.size(FFTFIT_results,1)+1))
...
fig2 = plt.figure()
ax2 = plt.gca()
ax2.plot(TOA_list[:,2], SNRs, marker='o', color='black')
...
fig3 = plt.figure()
ax3 = plt.gca()
ax3.plot(SNRs, TOA_list[:,3],  marker='o', color='black', markersize=2, linestyle='none')
plt.plot(np.linspace(0.4,12.7,num=100), (sampling_period*1e6)/np.linspace(0.4,12.7,num=100), linewidth=0.5, color='red', label='Ratio of sampling period to SNR')
ax3.set_xlabel('SNR')
