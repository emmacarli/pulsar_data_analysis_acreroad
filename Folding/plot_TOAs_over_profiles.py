...
for profile_number, folded_profile_path in enumerate(folded_profiles_paths): 
    ...
    TOA = TOA_list[profile_number, 2]
    #TOA_error = TOA_list[profile_number, 3]
    TOA_astropy = Time(TOA, format='mjd')
    #TOA_error_astropy = TimeDelta(TOA_error * units.us)
    
    
    TOA_from_start_of_observation = TOA_astropy - start_time_GPS_astropy - TimeDelta(2*60*60, format='sec')
    print(str(TOA_from_start_of_observation.to('s')))
...
#     
#     plt.savefig(path_to_folded_profiles+'Folded_Profile_'+str(start_time_GPS)+'.pdf') 
#     plt.close()
# =============================================================================
